...
After calculating session meters, delta meters, and lap delta, build the full context used by `profile.ShouldIgnoreLapIncrement(context)` and the generic distance-jump decision. These snapshots are structs, so correct decision timing does not introduce per-sample heap allocations.

Replace the iRacing zero-drop helper call with `profile.ShouldIgnoreTransientReset(context)`. Replace the low-speed line-wrap branch in the stateful updater with `profile.ShouldIgnoreLowSpeedLineWrap(context)`. Replace both calls to `LooksLikeIgnoredLapIncrement` with one cached `profile.ShouldIgnoreLapIncrement(context)` result. Keep `ShouldIgnoreDistanceJumpForIgnoredLapIncrement` as generic plugin logic using that cached decision and the existing half-track delta threshold.
